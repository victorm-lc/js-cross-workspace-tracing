...
If you need to dynamically route traces to different LangSmith workspaces based on runtime configuration (e.g., routing different users or tenants to separate workspaces), you can use workspace-specific LangSmith clients with `tracing_context` (Python) or pass a custom `client` to `traceable` / use `LangChainTracer` with callbacks (TypeScript).
